...
def train(model: FederatedModel, private_dataset: FederatedDataset,
    ...
    if args.rand_dataset:
        ...
        while not is_ok:
            if model.args.dataset == 'fl_officecaltech':
                ...
                selected_domain_list = ['caltech', 'caltech', 'dslr', 'amazon', 'dslr', 'dslr', 
                                        'dslr', 'webcam', 'caltech', 'amazon']
                ...
                selected_domain_list = ['mnist', 'mnist', 'svhn', 'usps', 'syn', 'usps', 'svhn', 'usps', 'mnist', 
                                        'syn', 'usps', 'usps', 'syn', 'svhn', 
                                        'usps', 'syn', 'svhn', 'syn', 'svhn', 'svhn']
            elif model.args.dataset == 'fl_pacs':
                domains_list = ['photo', 'art', 'cartoon', 'sketch']
                selected_domain_list = np.random.choice(domains_list, size=args.parti_num, replace=True, p=None)
                selected_domain_list = ['art', 'photo', 'art', 'sketch', 'art', 
                                        'sketch', 'photo', 'cartoon', 'sketch', 'cartoon']
    ...
    for epoch_index in range(Epoch):
        ...
        end_time = time.time()
        print('The ' + str(epoch_index) + ' Communcation Time:', round(end_time - start_time, 3))
        ...
        print('The ' + str(epoch_index) + ' Communcation Accuracy:', str(mean_acc), 
